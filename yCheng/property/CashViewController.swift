    ...
    */
    func submitMoney() -> Void {
        ...
        if moneyTextField?.text == "" {
            ...
            Drop.down("请输入要提现的金额", state: .error)
            return
        }else{
            if (moneyTextField?.text?.floatValue)! <= Float(0.0) {
                Drop.down("请输入有效的提现金额", state: .error)
                return
            }
        }
        
        let alert = UIAlertController(title: "确认", message: "确认需要提现\(moneyTextField?.text ?? "")元？", preferredStyle: .alert)
        let sureAction = UIAlertAction(title: "确定", style: .default) { (action) in
            
            Http.Http_PostInfo(post: url_boot+url_cash_out, parameters: moneyDic) { (responseDic) in
                print(responseDic)
...
extension CashViewController : UITableViewDataSource
{
...
extension CashViewController : CardViewControllerDelegate
{
...
extension CashViewController : UITableViewDelegate
{
